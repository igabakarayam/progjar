...
CLIENT_WORKERS = 50  # Number of client processes
...
def run_operation(filename, operation, workers):
    ...
    with Pool(processes=workers) as pool:
        # Prepare args as tuples: (worker_id, filename)
        args = [(i, filename) for i in range(workers)]
        results = pool.map(task_func, args)

    for success, elapsed, nbytes in results:
        if success:
            total_success += 1
            total_bytes += nbytes
...
if __name__ == '__main__':
    ...
    SERVER_WORKER_POOL_SIZE = 5  # Same as your server config
    for nomor, stats in report:
        ...
        # Server success/fail per file not tracked here, fill with '-'
        server_success = '-'
        server_fail = '-'
